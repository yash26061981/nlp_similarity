...
class Methods:
    ...
    def match_using_string_overlap(self, a_ans, u_ans, threshold):
        ...
        # print([a_ans], [u_ans], [ov_w], overlap_scores, [avg_ov_score], [difflib_score])
    ...
    def match_using_rhyming_words(self, a_ans, u_ans, threshold, best_match=False):
        score = 0.0
        is_similar = False
        if self.config.rhyme_pass_word_len < len(u_ans):
            return False, 0.0, None
        u_words = self.rhyming_words.suggest(u_ans)
        if u_words is not None:
            for uv in u_words:
                # conf = self.match_using_cosine_similarity(a_ans, uv)
                is_similar, conf, ov_w = self.match_using_string_overlap(a_ans, uv, threshold)

                if is_similar:
                    return True, conf, uv

        return is_similar, score, u_ans
