...
class GetSimilarity:
    def __init__(self, threshold=0.4):
        ...
        self.use_methods = ['DirectMatch', 'Cosine', 'NumWord', 'SynAnt', 'Partial', 'Rhyme', 'HybridMatch', 'WordForm']
        self.alternate_methods = ['SynAnt', 'WordForm', 'OtherOptionsAnswered']
        pass

    def use_method(self, args):
        actual_answer, utterance_answer, threshold = args.actual_answer, args.utterance_answer, args.threshold
        # self.methods.match_using_jaccard_similarity(actual_answer, utterance_answer, threshold)
        ...
        try:
            ...
            if dm_result.is_similar:
                ...
                if self.get_best_match and len(scores_list) > 0:
                    ...
                    if match_score == 0.0 and not(method_list[max_index] in self.alternate_methods):
